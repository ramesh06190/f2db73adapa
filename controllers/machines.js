...
exports.machines_create_Page = function(req, res) {
    console.log("create view")
    try{
    res.render('machinescreate', { title: 'Machine Create'});
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
// Handle building the view for updating a machine.
// query provides the id
exports.machines_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let results = await machines.findById(req.query.id)
    res.render('machinesupdate', { title: 'Machine Update', toShow: results });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
